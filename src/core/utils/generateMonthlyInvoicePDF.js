...
/**
 * invoice shape (from AdminDbController.getInvoiceDetailsForPartnerMonthly):
 * { partner: { id, name, phone, email, address, state, zipcode },
 *   month, from_date, to_date,
 *   items: [{ appointment_id, booking_time, appointment_date, service_name, important, amount }],
 *   total_bookings, total_amount }
 */
const generateMonthlyInvoicePDF = async (invoice) => {
  try {
    ...
    const periodFrom = new Date(invoice.from_date);
    const periodTo = new Date(invoice.to_date);
    const dueDate = new Date(periodTo);
    dueDate.setDate(dueDate.getDate() + 7);

    const invoiceNo = `INV-GLOUP-${String(invoice.month || "").replace(/-/g, "")}-${invoice.partner.id}`;
    ...
    <body>
      ...
      <div class="header">
        ...
        <div class="doc-title">
          ...
          <div class="meta-row"><span class="k">Billing Period</span>${formatDate(periodFrom)} &ndash; ${formatDate(periodTo)}</div>
      ...
      <div class="footer">
        <div class="terms">
          ...
          <ul>
            <li>Please review this monthly booking summary against your salon records within 7 days of the invoice date.</li>
    ...
    console.error("generateMonthlyInvoicePDF error:", error);
    throw error;
  }
};

export default generateMonthlyInvoicePDF;
